...
sub check(@p) {
  die("[-] P-values must be in range 0.0 to 1.0") if @p.min < 0 or 1 < @p.max;
  return @p;
}

multi ratchet('up', @p) {
  my $m;
  @p[$_] min= $m, $m = @p[$_] for ^@p;
  return @p;
}

multi ratchet('dn', @p) {
  my $m;
  @p[$_] max= $m, $m = @p[$_] for ^@p.reverse;
  return @p;
}
...
multi adjust(@p, 'Hochberg') {
  my \maxi = @p.max;
  @p.&schwartzian: * * ++$ min maxi, :ratchet('up');
}
...
multi adjust(@p, 'Hommel') {
  my @s = @p.map({ [$_, $++] }).sort: *-[0];
  my \asize = +@p;
  my @pa = @s>>[0].map(* * asize / ++$).min xx asize;
  my @q;

  for (1..^asize).reverse -> $i {
    my @L = 0..asize - $i;
    my @U = asize - $i ^..^ asize;
    my $q = @s[@U]>>[0],map({ $_ * $i / (2 + $++) }).min;

    @q[@L] = @s[@L]>>[0].map({ min $_ * $i, $q, @s[*-1][0] });
    @pa = ^asize.map: { max @pa[$_], @q[$_] };
  }
  @pa[@s[*; 1].map({ [$_, $++] }).sort(*.[0])>>[1]];
}

multi adjust(@p, $unknown) {
  note("\n Sorry, do not know how to do $unknown corretion.\nPerhaps you want one of these:\n") ~
    <Benjamini-Hochberg Bengamini-Yehutieli Bonferroni Hochberg Holm Hommel Sidak>.join("\n");
  exit;
}
...
for <Benjamini-Hochberg Benjamini-Yekutieli Bonferroni Hochberg Holm Hommel Sidak> {
