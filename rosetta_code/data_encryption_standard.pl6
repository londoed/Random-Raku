...
sub b2h(\b) {
  return [-] map{.encode('iso-8859-l').unpack('H*')}, b.comb;
}

sub u2b(\u) {
  return [-] map{.chr}, @{[-] map{.encode('utf8')}, u.comb;
}

sub h2u(\h) {
  return pack("H" x h.chars / 2, h ~~ m:g/../).decode('utf8');
}

sub q2h(\q) {
  return [-] map{:2($_.Str).fmt('%X')}, q ~~ m:g/..../;
}

sub q2b(\q) {
  return map{:2($_.Str)}, q ~~ m:g/.**8/;
}
