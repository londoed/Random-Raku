...
my $render = SDL.CreateRenderer($window, ACCELERATED +| PRESENTVSYNC);
...
main: loop {
  ...
  my $scale = (2 + 0.33 * abs(pi - $angle)) * $dim / $lines;

  $scale *= ($angle > pi) ?? (1 - $angle / τ) !! $angle / τ;
...
sub palette($l) {
  return (^$l).map: { hsv2rgb(($_ * 360 / $l & 360) / 360, 1, 1).list };
}
